...
This script is intended to run in CI (GitHub Actions) and fail fast when:
  ...
  - Any tracked `.cmd` file is not decodable as UTF-8, or contains a BOM.
...
Notes:
  ...
  - The UTF-8 requirement applies to `.cmd` only: those bytes are echoed to the
    Windows console under `chcp 65001`, so they must be UTF-8 to render correctly.
  - `.txt` files are opened in an editor (Notepad), not the console, so they are
    NOT forced to be UTF-8 no-BOM. `Instrucoes.txt` ships as UTF-8 with BOM,
    which modern Notepad detects reliably across locales/Windows versions.
#>

$repoRoot = Resolve-Path (Join-Path $PSScriptRoot '..')
# Ensure legacy code pages are available on .NET Core/PowerShell 7
...
function Validate-CmdEncoding {
    ...
    # Check for BOM markers - .cmd files should NOT have BOM
    if ($bytes.Length -ge 3 -and $bytes[0] -eq 0xEF -and $bytes[1] -eq 0xBB -and $bytes[2] -eq 0xBF) {
        Add-Failure -File $Path -Message 'Detected UTF-8 BOM in .cmd file. Save the file as UTF-8 without BOM.'
        return
    }

    if ($bytes.Length -ge 2 -and $bytes[0] -eq 0xFF -and $bytes[1] -eq 0xFE) {
        Add-Failure -File $Path -Message 'Detected UTF-16 LE BOM. Save the file as UTF-8 without BOM.'
        return
    }

    if ($bytes.Length -ge 2 -and $bytes[0] -eq 0xFE -and $bytes[1] -eq 0xFF) {
        Add-Failure -File $Path -Message 'Detected UTF-16 BE BOM. Save the file as UTF-8 without BOM.'
        return
    }
    ...
    try {
        ...
        Add-Failure -File $Path -Message 'Content is not valid UTF-8. Convert the file to UTF-8 without BOM (code page 65001).'
        return
    }
}
...
# Only `.cmd` files are echoed to the Windows console (under `chcp 65001`), so
# only they are required to be UTF-8 without BOM. `.txt` docs are opened in an
# editor and may use UTF-8 BOM (e.g. Instrucoes.txt ships as UTF-8 BOM for Notepad).
$cmdFiles = (git -C $repoRoot -c core.quotePath=false ls-files -z -- '*.cmd') -split "`0" | Where-Object { $_ }
foreach ($file in $cmdFiles) {
    Validate-CmdEncoding -Path $file
}
